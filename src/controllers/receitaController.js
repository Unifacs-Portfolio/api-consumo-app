...
class ReceitaController {
	async create(req, res) {
		...
		try {
			if (
				...
				throw new Error(
					"Campos obrigatórios: titulo, conteudo, idUsuario, tema, subtema",
				);
			}
			...
			const subtemas = Array.isArray(req.body.subtema)
				? req.body.subtema
				: [req.body.subtema];
			...
			for (const subtema of subtemas) {
				...
				if (temaSubtemaError && temaSubtemaError.code !== "PGRST116")
	...
	async getAll(req, res) {
		try {
			...
			const receitasComDetalhes = await Promise.all(
	...
	async getById(req, res) {
		try {
			...
			const subtemas =
				receita.correlacaoReceitas?.map((correlacao) => correlacao.subtema) ||
				[];
	...
	async update(req, res) {
		...
		try {
			...
			const temaAtualizado =
				correlacaoError || !correlacao ? req.body.tema : correlacao.tema;
			...
			if (req.files?.length > 0) {
				...
				if (fotosAntigas?.length > 0) {
					...
					await supabase.from("fotosReceitas").delete().eq("id", req.params.id);
				}
	...
	async getAllVerifiedByTheme(req, res) {
		try {
			...
			if (idPostError)
				return handleError(res, idPostError.message, 500, idPostError.details);
			...
			const { data: receitas, error } = await supabase
				...
				.in(
					"id",
					idPost.map((post) => post.idReceita),
				)
				.eq("isVerify", true);
		...
		try {
			...
			if (correlacaoError) {
				...
				return res
					.status(500)
					.json({
						error: `Erro ao buscar correlações de receitas: ${correlacaoError.message}`,
					});
			}
			...
			const idsReceitas = [
				...new Set(correlacoes.map((correlacao) => correlacao.idReceita)),
			];
			...
			if (receitasError) {
				...
				return res
					.status(500)
					.json({
						error: `Erro ao buscar as receitas: ${receitasError.message}`,
					});
			}
