...
class DicaController {
	async create(req, res) {
		try {
			...
			if (dicaError)
				return handleError(res, dicaError.message, 500, dicaError.details);
			...
			return res
				.status(201)
				.json({ message: "Dica criada com sucesso", data: dicaData[0] });
		} catch (e) {
	...
	async update(req, res) {
		try {
			...
			if (updateError)
				return handleError(res, updateError.message, 500, updateError.details);
	...
	async getDica(req, res) {
		try {
			...
			if (dicasError) {
				...
				return res
					.status(500)
					.json({ error: `Erro ao buscar as dicas: ${dicasError.message}` });
			}
