...
class IngredienteController {
	async store(req, res) {
		try {
			...
			return res
				.status(201)
				.json({ message: "Ingrediente adicionado com sucesso", data: data });
		} catch (e) {
	...
	async show(req, res) {
		try {
			...
			if (error || !ingrediente) {
				handleError(
					res,
					`O ingrediente com o Id ${req.params.ingredienteId} não foi encontrado.`,
					404,
	...
	async update(req, res) {
		try {
			if (!valid) {
				return res
					.status(400)
					.json({ message: "Esse ingrediente não é válido", errors: errors });
			}
			...
			if (!updatedIngrediente) {
				handleError(
					res,
					`O ingrendiente com o id ${req.params.ingredienteId} não foi encontrado.`,
					404,
					"Ingrediente não encontrado",
				);
			}
